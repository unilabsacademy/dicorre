...
// Cache for consistent value generation across sessions
 ...
 */
function getCachedValue(fieldName: string, originalValue: string): string {
 ...
 */
export function createValueReplacementHandler() {
  return (element: DicomElement, options: any) => {
    const tagName = element.keyword || element.name || ''
    ...
    switch (tagName) {
      case 'PatientName':
        ...
        break
      case 'PatientID':
        newValue = getCachedValue('PatientID', originalValue)
        break
      case 'StudyID':
        newValue = getCachedValue('StudyID', originalValue)
        break
      case 'AccessionNumber':
        ...
        newValue = getCachedValue('AccessionNumber', keyValue)
        break
      case 'StudyInstanceUID':
        newValue = getCachedValue('StudyInstanceUID', originalValue)
        break
      case 'SeriesInstanceUID':
        newValue = getCachedValue('SeriesInstanceUID', originalValue)
        break
      case 'SOPInstanceUID':
    ...
    if (!newValue) {
      switch (tagNumber) {
        ...
        case '00080050': // Accession Number
          newValue = getCachedValue('AccessionNumber', originalValue)
          break
      }
    }
 ...
 */
export function getAllSpecialHandlers(jitterDays: number = 31, tagsToRemove: string[] = []) {
  return [
